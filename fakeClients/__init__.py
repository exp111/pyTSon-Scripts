...
class fakeClients(ts3plugin):
    ...
    def onMenuItemEvent(self, schid, atype, menuItemID, selectedItemID):
        if atype == ts3defines.PluginMenuType.PLUGIN_MENU_TYPE_CLIENT:
            ...
            if menuItemID == 1: sendCommand(self.name, "notifyclientleftview cfid={} ctid=0 reasonid=8 reasonmsg=disconnected clid={}".format(cid, selectedItemID), schid, True, False)
            elif menuItemID == 2: sendCommand(self.name, "notifyclientleftview cfid={} ctid=0 reasonid=5 reasonmsg=kicked clid={} invokerid=0 invokername=Server invokeruid".format(cid, selectedItemID), schid, True, False)
            elif menuItemID == 3: sendCommand(self.name, "notifyclientleftview cfid={} ctid=0 reasonid=6 reasonmsg=ban clid={} invokerid=0 invokername=Server invokeruid bantime=0".format(cid, selectedItemID), schid, True, False)
            elif menuItemID == 4: sendCommand(self.name, "notifyclientleftview cfid={} ctid=0 reasonid=3 reasonmsg=DDoS clid={}".format(cid, selectedItemID), schid, True, False)
        elif atype == ts3defines.PluginMenuType.PLUGIN_MENU_TYPE_GLOBAL:
            for client in self.fakeclients:
                sendCommand(self.name, "notifyclientleftview cfid={} ctid=0 reasonid=6 reasonmsg=ban clid={} invokerid=0 invokername=Server invokeruid bantime=0".format(client[1], client[0]), schid, True, False)
            self.fakeclients = []
    ...
    def onClientMoveEvent(self, schid, clientID, oldChannelID, newChannelID, visibility, moveMessage):
        ...
        sendCommand(self.name, "notifyclientupdated clid={} client_is_channel_commander=1".format(clientID), schid, True, False) #  client_country=JP client_is_recording=1 client_platform=Windoof
